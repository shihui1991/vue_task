<template>
    <el-row :gutter="20">
        <el-col :lg="isShowMenu ? 12 : 24" :md="24">
            <el-card class="box-card">
                <div slot="header" class="clearfix">
                    <span>菜单结构</span>
                    <el-button-group style="float:right;">
                        <el-button size="mini" type="success" icon="el-icon-circle-plus-outline" @click="showMenu">添加菜单</el-button>
                        <el-button size="mini" type="danger" icon="el-icon-delete" @click="delAll" >批量删除已选</el-button>
                    </el-button-group>
                </div>
                <div class="text item">
                    <div class="tip">
                        <el-input placeholder="输入关键字进行过滤" prefix-icon="el-icon-search" v-model="filterTreeMenu"></el-input>
                    </div>
                    <el-tree
                            :props="defaultProps"
                ...
                <div class="text item">
                    <el-form ref="formItems" :model="formItems" label-width="120px" :rules="rules" show-message>
                        ...
                        <el-form-item label="路由：" prop="uri">
                            <el-input v-model="formItems.uri" />
                        </el-form-item>
                        <el-form-item label="是否限制：" prop="is_ctrl">
                            <el-radio-group v-model="formItems.is_ctrl">
                                <el-radio v-for="(title,val) in isCtrlMap" :label="parseInt(val)" border>{{ title }}</el-radio>
                            </el-radio-group>
                        </el-form-item>
                        ...
                        <el-form-item label="描述：" prop="infos">
                            <el-input v-model="formItems.infos" />
                        </el-form-item>
                        ...
                        <el-form-item>
                            ...
                            <el-button @click="hideMenu">取消</el-button>
                        </el-form-item>
                    </el-form>
                </div>
            </el-card>
...
<script>
    import {getList,store,del} from '@/api/menu'
    ...
    export default {
            ...
            return {
                ...
                rules:{
                    parent_id:[
                        ...
                        {type: 'integer',min:0, message: '请选择上级菜单', trigger: 'blur'},
                    ],
                    name: [
                    ...
                    infos: [
                        { max: 255, message: '最多输入 255 个字符', trigger: 'blur' }
                    ],
                    uri: [
                        { required: true, message: '请输入名称', trigger: 'blur' },
                        { min: 1, max: 255, message: '长度在 1 到 255 个字符', trigger: 'blur' }
                    ...
                    is_ctrl: [
                        ...
                        { type:'enum', enum: [0,1], message: '请选择是否限制', trigger: 'change' }
                    ],
                    status: [
        ...
        methods: {
            ...
            filterNode(val, data) {
                if(!val) return true;
                return data.name.indexOf(val) !== -1 || data.uri.indexOf(val) !== -1;
            },
            renderContent(h, { node, data, store }) {
                return (
                    ...
                    <span>{data.uri}</span>
                    <span>{this.isCtrlMap[data.is_ctrl]} | {this.statusMap[data.status]}</span>
                    <span>
                    <el-button-group>
            ...
            add(node){
                this.showMenu();
                this.title = '添加下级菜单 - ' + node.data.name;
            ...
            info(node){
                this.showMenu();
                this.title = '菜单详情 - ' + node.data.name;
                ...
                this.$confirm('是否确认删除【'+node.data.name+'】？', '温馨提示', {
                    ...
                    del({ids:[node.data.id]}).then(response =>{
                        for(let row of node.data.children){
                            this.$refs.menuTree.append(row,node.parent)
                        }
            ...
            submitForm(form) {
                this.$refs[form].validate((valid) => {
                    ...
                    store({input:this.formItems},{id:this.formItems.id}).then(response =>{
                        let node = this.$refs.menuTree.getNode(nodeId);
                        if(response.data.isNew){
                            response.data.row.children = [];
                            node ? this.$refs.menuTree.append(response.data.row,node) : this.$refs.menuTree.data.push(response.data.row);
                        }else{
